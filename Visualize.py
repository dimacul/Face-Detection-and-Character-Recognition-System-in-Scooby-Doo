...
"""
    ...
    (doar numele, nu toata calea).
    """
def show_detections_with_ground_truth_task2(detections, scores, file_names, char_name, params: Parameters):
    

    # for char_name in ['fred', 'daphne', 'velma', 'shaggy']:
    ground_truth_bboxes = np.loadtxt(os.path.join(params.path_annotations, f'task2_{char_name}_gt_validare.txt'), dtype='str')
    test_images_path = os.path.join(params.dir_test_examples, '*.jpg')
    test_files = glob.glob(test_images_path)

    for test_file in test_files:
        short_file_name = ntpath.basename(test_file)
        cale_imagine_in_salveazaFisiere = Path(fr"C:\Users\crist\Documents\FACULTATE\ANUL 3\CAVA\LAB\TEMA2\341_Dima_Cristian_sursa\data\task2\salveazaFisiere\{short_file_name}")
        if os.path.exists(cale_imagine_in_salveazaFisiere):
            image = cv.imread(cale_imagine_in_salveazaFisiere)
        else:
            image = cv.imread(test_file)
        ...
        for idx, detection in enumerate(current_detections):
            cv.rectangle(image, (detection[0], detection[1]), (detection[2], detection[3]), (0, 0, 255), thickness=1)
            cv.putText(image, f'{char_name}: ' + str(current_scores[idx])[:4], (detection[0], detection[1]),
                    cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
        annotations = ground_truth_bboxes[ground_truth_bboxes[:, 0] == short_file_name]
        ...
        cv.imwrite(os.path.join(params.dir_save_files, short_file_name), image)
        # print('Apasa orice tasta pentru a continua...')
        # cv.imshow('image', np.uint8(image))
        # cv.waitKey(0)
